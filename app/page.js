...
export default function Home() {
  ...
  const [useCloudEnv, setUseCloudEnv] = useState(false);
  const [websiteTarget, setWebsiteTarget] = useState('https://creditkarma.com');
  const [instructionsPrompts, setInstructionsPrompts] = useState('Browse around the site and click on credit cards.\nReview credit cards in 3 different categories\nReview types of personal loans');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (mode === 'multiple') {
        payload.numSessions = parseInt(numSessions);
      }
